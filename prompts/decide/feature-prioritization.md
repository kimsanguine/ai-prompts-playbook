...
| Job | 기능 후보를 팀 PM용 RICE와 1인 빌더용 실행 점수로 정렬하기 |
| Best for | 1인 빌더, PM, CPO, Product Trio |
| Required inputs | 기능 후보 목록, 문제 정의, 비즈니스 목표, 대략적인 노력 추정 |
| Optional inputs | 사용자 수, 매출 영향, 기술 리스크, 규제/보안 제약, 출시 목표일 |
| Output | RICE 점수, Solo Builder Score, 지금 할 것/하지 말 것/먼저 검증할 것 |
| Next prompts | `prd-for-ai-feature` → `sprint-kickoff` → `executive-briefing` |
| Evaluation | [`evals/scenarios/core.yaml`](../../evals/scenarios/core.yaml)의 `starter-feature-prioritization` |

## ✅ Output Contract

AI의 답변은 반드시 다음을 포함해야 합니다:

1. **우선순위 표**: RICE 기준 점수와 순위
2. **Solo Builder Score**: 속도, 수익 검증 가능성, 배포 가능성, 유지보수 부담, 차별화 신호
3. **점수 근거**: 각 숫자가 왜 나왔는지 1-2문장 설명
4. **지금 할 것**: 다음 1주 또는 다음 스프린트에 실행할 항목
5. **하지 말 것**: 지금 만들면 안 되는 기능과 이유
6. **먼저 검증할 것**: Confidence가 낮아 실험이 필요한 항목
7. **의존성/리스크**: 기술, 데이터, 법무, 운영 제약
8. **다음 프롬프트 연결**: 어떤 기능을 `prd-for-ai-feature`에 넘길지

## 👥 사용 모드

| 모드 | 우선순위 기준 | 좋은 결과 |
|------|---------------|-----------|
| 1인 빌더 | 1주 안에 만들고 검증할 수 있는가 | 작은 MVP 범위와 포기 기준 |
| PM | 사용자 영향과 팀 실행 가능성이 균형 잡혔는가 | 스프린트 후보와 검증 태스크 |
| CPO | 전략 목표와 리소스 배분에 맞는가 | 투자/보류/중단 판단 |

## 🧪 실패 기준

- 점수만 있고 근거가 없으면 실패입니다.
- Confidence가 낮은 기능을 검증 없이 1순위로 두면 실패입니다.
- “하지 말 것”이 없으면 우선순위가 아니라 할 일 목록입니다.
...
이 프롬프트는 **5단계 Reasoning Strategy** 구조입니다. 두 가지 방식으로 쓸 수 있습니다:
