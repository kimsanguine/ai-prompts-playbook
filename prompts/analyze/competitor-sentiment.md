...
| 항목 | 내용 |
|------|------|
| ID | `competitor-sentiment` |
| Job | 경쟁사 고객의 불만과 감정에서 우리 제품의 기회 찾기 |
| Best for | 1인 빌더, PM, CPO, Growth/Marketing |
| Required inputs | 경쟁사명, 제품/아이디어 설명, 리뷰 20개 이상 또는 리뷰를 찾을 데이터 소스 |
| Optional inputs | 우리 제품의 현재 포지션, 타깃 고객, 가격대, 국내/글로벌 시장 구분 |
| Output | 불만 클러스터, 페르소나, 감정 점수, 근거 리뷰, 기회 매트릭스, MVP 아이디어 |
| Next prompts | `problem-framing` → `feature-prioritization` → `prd-for-ai-feature` |
| Evaluation | [`evals/scenarios/core.yaml`](../../evals/scenarios/core.yaml)의 `starter-competitor-sentiment` |

## ✅ Output Contract

AI의 답변은 반드시 다음을 포함해야 합니다:

1. **핵심 결론**: 경쟁사가 가장 크게 놓치고 있는 고객 문제 3개
2. **근거 리뷰**: 각 결론을 뒷받침하는 실제 리뷰 인용 또는 데이터 출처
3. **불만 클러스터**: 가격, 기능, UX, 성능, 지원, 신뢰 등 주제별 분류
4. **페르소나**: 불만을 느끼는 고객군, JTBD, 감정 점수
5. **기회 매트릭스**: 영향력과 실현 가능성 기준의 우선순위
6. **1인 빌더용 MVP 아이디어**: 1-2주 안에 검증 가능한 작은 제품/기능 후보
7. **가정과 불확실성**: 데이터가 부족하거나 추정한 부분
8. **다음 프롬프트 연결**: 어떤 결과를 `problem-framing`에 넘길지

데이터가 부족하면 `No-data mode`로 전환해 공개 리뷰를 찾을 위치, 검색어, 최소 수집 기준을 먼저 제안하세요.

## 👥 사용 모드

| 모드 | 초점 | 좋은 결과 |
|------|------|-----------|
| 1인 빌더 | 빠르게 만들 수 있는 틈새 기회 | 1주 안에 테스트할 MVP 후보와 랜딩/수동 검증 아이디어 |
| PM | 기능/UX 개선 기회 | 문제 정의와 우선순위 결정에 넘길 근거 있는 기회 |
| CPO | 포지셔닝과 전략적 차별화 | 경쟁사가 구조적으로 못 푸는 시장 기회 |

## 🧪 실패 기준

- 리뷰 근거 없이 일반적인 기능 비교만 하면 실패입니다.
- 페르소나가 직무명 나열에 그치고 JTBD가 없으면 실패입니다.
- “그래서 무엇을 만들거나 바꿔야 하는가?”가 없으면 실패입니다.
...
이 프롬프트는 **9단계 Reasoning Strategy** 구조입니다. 두 가지 방식으로 쓸 수 있습니다:
